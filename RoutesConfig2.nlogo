...
to move
  ask shops [
    ...
    ;if maisons-dispo < 2 [demenage-job]
    ;if centre-dispo < 1 or 
  ...
  ask houses2 [
    ...
    if other-similar2 < Similar-wanted2 [demenage-job]
    if other-diff2 < 2 [demenage-job]
    if nb-shops < 1 [demenage-job] ; condition ajoutée pour éviter que les houses2 ne soient pas trop éloignées de tout
    ]
  ask houses1 [
    let emplois-dispo count shops in-radius 7
    if emplois-dispo < Job-in-radius [demenage-house]
    let other-similar1 count houses1 in-radius 4
...
to demenage-init
  move-to one-of patches with [count turtles-here = 0] ; + rien sur la cellule voisine?
end

to demenage-house
  move-to one-of patches with [count turtles-here + count houses1-here <= 1]
end

to demenage-job
  move-to one-of patches with [count shops-here + count centres-here <= 3]
 ; move-to one-of patches with [count shops-here + count houses2-here + count centres-here = 0]
end
